...
const pos = data.split('\n').flatMap((x, i) => ([...x.matchAll(/\d+/g)].map(y => ({ i: y.index, j: i, val: y[0] }))));
...
let acc = 0;
for (const p of pos) {
		const surrounding = (lines[p.j - 1] ?? []).slice(Math.max(0,p.i-1), p.i + p.val.length + 1)
		.concat(lines[p.j][Math.max(0,p.i-1)] ?? "")
		.concat(lines[p.j][p.i + p.val.length] ?? "")
		.concat((lines[p.j + 1] ?? []).slice(Math.max(0,p.i-1), p.i + p.val.length + 1));
		if (surrounding.some(x => RegExp(/[^\d.]/).exec(x))) {
			acc+=+p.val;
			console.log(p.val + "YES")
		} else console.log(p.val + "n");
		//console.log(p.val, surrounding)
}
console.log(acc)
