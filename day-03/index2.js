const data = `.....664...998........343...............851............................2............414.....................3....................948.164....
...
const pos = data.split('\n').flatMap((x, i) => ([...x.matchAll(/\d+/g)].map(y => ({ j: y.index, i: i, val: y[0] }))));

for (let i = 0; i < lines.length; i++)
	for (let j = 0; j < lines[i].length; j++) {
		if (lines[i][j] === "*") stars.push({i, j});
	}

const between = (a, b, c) => a >= b && a < c;
let acc = 0;
for (const star of stars) {
	const topLeft = pos.filter(x => x.i === star.i - 1).filter(pos => star.j - 1 >= pos.j && star.j - 1 < pos.j + pos.val.length);
	const top = pos.filter(x => x.i === star.i - 1).filter(pos => star.j >= pos.j && star.j < pos.j + pos.val.length);
	const topRight = pos.filter(x => x.i === star.i - 1).filter(pos => star.j + 1 >= pos.j && star.j + 1 < pos.j + pos.val.length);
	
	const midLeft = pos.filter(x => x.i === star.i).filter(pos => star.j - 1 >= pos.j && star.j - 1 < pos.j + pos.val.length);
	const midRight = pos.filter(x => x.i === star.i).filter(pos => star.j + 1>= pos.j && star.j + 1 < pos.j + pos.val.length);
	const bottomLeft = pos.filter(x => x.i === star.i + 1).filter(pos => star.j - 1 >= pos.j && star.j - 1 < pos.j + pos.val.length);
	const bottom = pos.filter(x => x.i === star.i + 1).filter(pos => star.j >= pos.j && star.j < pos.j + pos.val.length);
	const bottomRight = pos.filter(x => x.i === star.i + 1).filter(pos => star.j + 1 >= pos.j && star.j + 1 < pos.j + pos.val.length);
	const all = [...new Set([top, topLeft, topRight, midLeft, midRight, bottomLeft, bottom, bottomRight].flat())]
	if (all.length === 2) acc+=all.map(x => +x.val).reduce((l,c)=>l*c,1);
