...
for (const line of input.split("\n")) {
	const num = +line.match(/Game (\d+)/)[1];
	const rounds = line.match(/: (.+)/)[1].split("; ");
	let maxes = [0,0,0]
	for (const round of rounds) {
		const balls = round.split(", ").map(x => x.split(" ").reverse()).map(x=>[x[0],+x[1]]);
		const ballMap = Object.fromEntries(balls);
		if (ballMap.red > maxes[0]) maxes[0] = ballMap.red;
		if (ballMap.green > maxes[1]) maxes[1] = ballMap.green;
		if (ballMap.blue > maxes[2]) maxes[2] = ballMap.blue;

	}
	acc+=maxes[0] * maxes[1] * maxes[2];
}
console.log(acc)